...
Note how the service worker fails to register when we run the app in "dev" mode (`npm run dev`), but works fine in production or preview builds (`npm run build` → `npm run preview`).
...
And the requested URL (`/service-worker.js`) gives a "302 Found".
